...
class VectorHeat(object):
    ...
    def __call__(self, data):
        ...
        pos, face, edge_index = data.pos.cpu().numpy(
        ), data.face.cpu().numpy().T, data.edge_index.cpu().numpy().T
        ...
        vh_weights = vh.weights(pos, face, sample_idx,
                                np.arange(len(sample_idx)))

        # Unpack results and create torch tensors.
        loop_idx = torch.nonzero(
            data.edge_index[0] == data.edge_index[1]).view(-1)
