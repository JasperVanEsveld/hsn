...
from visualize import show_neighbours
...
class MultiscaleRadiusGraph(object):
    ...
    def __init__(self, ratios, radii, max_neighbours=512, sample_n=None, loop=False, flow='source_to_target', cache_file=None):
        ...
        if self.get_cache:
            self.edge_index, self.node_mask, self.edge_mask = torch.load(
                cache_file)
    ...
    def __call__(self, data):
        ...
        if self.sample_n is not None and not self.sample_n > data.pos.size(0):
            sample_idx = fps(pos, batch, ratio=self.sample_n /
                             data.pos.size(0)).sort()[0]
        else:
        ...
        for i, r in enumerate(self.ratios):
            ...
            if r == 1:
                ...
                idx_vh, labels_vh = sample_idx[pool_idx].cpu(
                ).numpy(), np.arange(pool_idx.size(0))
                pool_neigh = torch.from_numpy(vh.nearest(pos_vh, face_vh, idx_vh, labels_vh)).round(
                ).long().clamp(0, pool_idx.size(0) - 1).view(-1)

                # Add edges for pooling
                edge_index = torch.cat((edge_index, torch.stack(
                    (original_idx[pool_idx][pool_neigh][sample_idx], original_idx), dim=0)), dim=1)
                # Add corresponding mask entries for pooling edges
                edge_mask = torch.cat(
                    (edge_mask, (torch.ones(sample_idx.size(0)) * (0b1 << (i * 2))).long()))

            # Sample nodes
            sample_idx, original_idx, pos, batch = sample_idx[pool_idx], original_idx[
                pool_idx], pos[pool_idx], batch[pool_idx] if batch is not None else None
            ...
            edge_mask = torch.cat(
                (edge_mask, (torch.ones(radius_edges.size(1)) * (0b1 << (i * 2 + 1))).long()))
