...
class ScaleMask(object):
    ...
    def __call__(self, data):
        ...
        mask = mask_idx(self.edge_lvl, data.edge_mask)
        ...
        node_idx_ = torch.nonzero(data.node_mask_ >= (
            self.node_lvl - self.pooling)).view(-1)
        ...
        if hasattr(data, 'connection'):
            ...
            res.weight = data.node_weight[res.edge_index[1]] if hasattr(
                data, 'node_weight') else data.weight[mask]
        return res
